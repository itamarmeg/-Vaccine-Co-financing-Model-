...
## Ghana figures
```{r Ghana figure no longer in use, fig.height=10, fig.width=8, echo=FALSE, eval=FALSE}
# Add plotting themes
library(ggthemes)
...
#  facet_grid(threshold~year, scales = "free") +
  geom_line(data = gdp_data_ghana %>% 
              filter(country == "Ghana" &
                       year %in% cea_year) %>%
              filter(cost < max_ghana_x),# | cea_line < max_ghana_y),
            mapping = aes(x = cost/1000000, y = cea_line/1000)) +
  geom_point(data = data_ghana,
             mapping = aes(x = cost/1000000, y = benefit/1000, color = vaccine_cea_paper)) +
  geom_segment(data = data_ghana,
               aes(x = cost/1000000, xend = (cost * domestic_p)/1000000, y = benefit/1000, yend = benefit/1000),
               arrow = arrow(length = unit(0.1, "cm"))) +
  scale_x_continuous(name = "Cost (in millions USD 2018)", labels = scales::dollar) +
  scale_y_continuous(name ="DALYs averted (in thousdands)", labels = scales::comma) +
...
p1 <- ggplot() +
  ...
  geom_segment(data = data_ghana,
               aes(x = cost/1000000, xend = (cost * domestic_p)/1000000, 
  ...
  scale_y_continuous(name ="DALYs averted (in thousdands)", 
                     labels = scales::comma) +
  # Remove color legend name
  scale_color_discrete(name="") +
  # Display color legend in two rows
  guides(color = guide_legend(nrow = 2, byrow = TRUE)) +
...
p1 +
  ...
  labs(caption = paste(
    ...
    "Points costs and benefits are extracted from cost-effectiveness (CE) studies;",
...
```{R cross country figure, fig.width=8, fig.height=10, echo=FALSE}
# All country data
...
# Save figure
ggsave("figures/country_compare.png", p2, dpi = 300, 
       width = 8*2.5, height = 10*2.5, units = "cm")
...
# Create and save figure with appendix caption
ggsave("figures/A2_country_compare.png", dpi = 300, 
       width = 8*2.5, height = 10*2.5, units = "cm")
```